...
app.get('/api/reporte-ventas', async (req, res) => {
  const cacheKey = "reports:sales_by_category";
  try {
    // 1. Consulta a la Caché: El endpoint verifica primero si la clave existe en Redis.
    const cachedData = await redisClient.get(cacheKey);

    // 2. Cache HIT: Si el dato existe, se retorna inmediatamente al cliente (se evita ir a la DB).
    if (cachedData) {
      console.log(`[Cache-Aside] HIT - Retornando datos desde Redis para la clave: ${cacheKey}`);
      return res.status(200).json(JSON.parse(cachedData));
    }

    // 3. Cache MISS (Consulta a la DB): Si el dato NO existe, el sistema realiza la consulta a la base de datos principal.
    console.log(`[Cache-Aside] MISS - Consultando MongoDB para la clave: ${cacheKey}`);
    ...
    // 4. Población de la Caché: Guardamos el resultado obtenido de la base de datos en Redis con un TTL (60s).
    await redisClient.set(cacheKey, JSON.stringify(reporte), {
      EX: 60
    });

    // 5. Devolver la respuesta final al cliente en todos los flujos.
